...
class LoginUserForm(AuthenticationForm):
    ...
    def clean_username(self):
        ...
        if username == "admin":
            messages.add_message(self.request, messages.SUCCESS, "Hoş Geldiniz Sn. Admin")

        return username
    
    # def confirm_login_allowed(self, user):
    #     if user.username.startswith("b"):
    #         raise forms.ValidationError("Bu Kullanıcı Adı İle Login Olamazsınız.")
...
class NewUserForm(UserCreationForm):
    ...
    def clean_email(self):
        ...
        if User.objects.filter(email = email).exists():
            self.add_error("email","BUu Email Adresi Daha Önce Kullanışmış.")
